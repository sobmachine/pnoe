...
// Encode 8x8 luma block
void encode_luma_block(const uint8_t y[BLOCK_SIZE][BLOCK_SIZE], int16_t qzz[ZZ], uint8_t *mean_out) {
    ...
    *mean_out = m;
    ...
    quantize_block(dct_zz, qzz, quant_luma_zz);
    qzz[0] = m;
}

// Decode 8x8 luma block
void decode_luma_block(const int16_t qzz[ZZ], uint8_t y[BLOCK_SIZE][BLOCK_SIZE], uint8_t mean_val) {
    // Dequantize
    uint8_t mean_value = qzz[0];
    float dct_zz[ZZ];
    dequantize_block(qzz, dct_zz, quant_luma_zz);
    dct_zz[0] = 0;
    // Unzigzag
